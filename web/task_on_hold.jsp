<%-- 
    Document   : task_on_hold
    Created on : 11 Dec, 2019, 11:14:05 AM
 ...
 <script type="text/javascript"> window.onload = alertName; </script>
    ...
    <body>
        ...
        <div align='left' style="">
            <form name="frm2" action="task_on_hold.jsp" method="post">
            <h2>Filter client id</h2>
            <input type="text" name="client_fil"/>
            <input type="submit" value="Filter"/>
            </form>
        <form action="unhold_task" method="post">
        <table>
            <thead>
        ...
        <%
           ...
           Class.forName("com.mysql.jdbc.Driver");
            ...
            if(client_fil!=null)
            {
                rs=st.executeQuery("select * from tasks where creator = '"+username+"' and status = \"hold\" and client_id = '"+client_fil+"' ");
                session.setAttribute("filter",client_fil);
            }
            else
            {
                rs=st.executeQuery("select * from tasks where creator = '"+username+"' and status = \"hold\" ");
                session.setAttribute("filter","null");
            }
            String cre_date,task,due_date,client,assigned_to,id,type;
            int flag=0;
            while(rs.next())
            {
                flag=flag+1;
                ...
                if(type.equals("t"))
                {
                    rt=sr.executeQuery("select * from temp_client where id = '"+client+"' ");
                }
                else
                {
                    rt=sr.executeQuery("select * from client where id = '"+client+"' ");
                }
                while(rt.next())
                {
                ...
                out.print("<td width='"+180+"'><input type=\"submit\" value=\"Unhold\" name=\"id'"+flag+"'\"></td>");
                out.print("</tr>");
