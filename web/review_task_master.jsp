<%-- 
    Document   : review_task
    Created on : 4 Dec, 2019, 12:54:14 PM
 ...
 <script type="text/javascript"> window.onload = alertName; </script>
    ...
    <body>
        ...
        <div align='left' style="">
            <form name="frm2" action="review_task_master.jsp" method="post">
            <h2>Filter client id</h2>
            <input type="text" name="client_fil"/>
            <input type="submit" value="Filter"/>
            </form>
        <form name="frm" action="approve_task" method="post">
        <table>
            <thead>
        ...
        <%
           ...
           Class.forName("com.mysql.jdbc.Driver");
            ...
            if(client_fil!=null)
            {
                rs=st.executeQuery("select * from tasks where creator = '"+username+"' and status = \"for review\" and client_id = '"+client_fil+"' ");
                session.setAttribute("filter",client_fil);
            }
            else
            {
                rs=st.executeQuery("select * from tasks where creator = '"+username+"' and status = \"for review\" ");
                session.setAttribute("filter","null");
            }
            String cre_date,task,due_date,client,assigned_to,id1,type,remark;
            int id=0;
            while(rs.next())
            {
                id=id+1;
                ...
                out.print("<td width='"+180+"'>"+assigned_to+"</td>");
                out.print("<td width='"+180+"'><input type=\"text\" name=\"remark\" value=\""+remark+"\"</td>");
                out.print("<td width='"+180+"'><input type=\"submit\" value=\"Approved\" name=\"id'"+id+"'\"></td>");
                out.print("<td width='"+180+"'><input type=\"submit\" value=\"Disapproved\" formaction=\"disapprove_task\" name=\"id'"+id+"'\"></td>");
                out.print("</tr>");
            }
            %>
            </table>
